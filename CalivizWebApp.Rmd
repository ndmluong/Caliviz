...
<h4> /!\\ Fusion en cours avec le premier prototype Streamlit du défi Caliviz (Data challenge Green Data for Health) </h4>

<p style="color:#FF9940"> *Dernière mise à jour: le 14 juin 2023 à 12:00* </p>
...
inputPanel(
  selectInput("eat2_fam_sub_pres", label = "Substance",
              choices = tapply(d_subs_pres$`Substance`, d_subs_pres$`Famille`, function(x) x))
)
...
```{r echo=FALSE}
renderText({ d_subs_pres$Abstract[d_subs_pres$Substance == input$eat2_fam_sub_pres] })
```

## Informations détaillées

```{r echo=FALSE}
renderText({ d_subs_pres$GeneralInfo[d_subs_pres$Substance == input$eat2_fam_sub_pres]})
```

<h3> Caractérisation du danger </h3>

```{r echo=FALSE}
renderText({ d_subs_pres$Hazards[d_subs_pres$Substance == input$eat2_fam_sub_pres] })
```

<h3> Evaluation et caractérisation du risque </h3>

```{r echo=FALSE}
renderText({ d_subs_pres$Risks[d_subs_pres$Substance == input$eat2_fam_sub_pres] })
```
...
```{r echo=FALSE}
renderText({ d_subs_pres$ToxicoRisk_EAT2[d_subs_pres$Substance == input$eat2_fam_sub_pres] })
```

<h4> Risques nutritionnels </h4>

```{r echo=FALSE}
renderText({ d_subs_pres$NutriRisk_EAT2[d_subs_pres$Substance == input$eat2_fam_sub_pres] })
```

<h4> Actions et besoin de recherche </h4>

```{r echo=FALSE}
renderText({ d_subs_pres$Actions_EAT2[d_subs_pres$Substance == input$eat2_fam_sub_pres] })
```
...
*(en cours de construction, fonctionnalité disponible temporairement sur [le premier prototype Streamlit du projet Caliviz, onglet Aliments contributeurs](https://caliviz.streamlit.app/))*
